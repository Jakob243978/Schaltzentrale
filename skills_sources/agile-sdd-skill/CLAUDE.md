# agile-sdd-skill — self-contained SDD+PO-Skill-Projekt

Dieser Skill steuert, wie ein KI-Agent eigenstaendig Software entwickelt (Spec-First, Tickets, ADRs, Verifier-Pass, Living Docs). Seit dem Dezentralisierungs-Umbau (2026-07-12) ist er
...
> `~/.claude/skills/agile-sdd-skill/` via `setup.ps1` (robocopy /MIR) der Schaltzentrale.
...
SDD-Config: `docs/sdd-config.yaml` (`manual_verify_required: false` — headless Methodik-/Tooling-Skill). Hinweis: Doc-/Methodik-Skill — keine pytest-Suite; Verifikation via Review + Dogfood in konsumierenden Projekten.
